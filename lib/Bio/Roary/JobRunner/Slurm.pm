package Bio::Roary::JobRunner::Slurm;

# ABSTRACT: Use GNU Parallel with Slurm

=head1 SYNOPSIS

 Execute a set of commands using GNU parallel
   use Bio::Roary::JobRunner::Slurm;
   
   my $obj = Bio::Roary::JobRunner::Slurm->new(
...
sub run {
    my ($self) = @_;
	  ...
	  my($fh, $temp_command_filename) = tempfile();
          for (@{ $self->commands_to_run }) {
              s/^/srun --exclusive -N1 -n1 /
          }
    ...
    for my $command_to_run(@{ $self->commands_to_run })
    {
       $self->logger->info($command_to_run);
		...
		my $parallel_command  = "salloc -n".$self->cpus." parallel --gnu -j ".$self->cpus." < ".$temp_command_filename ;
